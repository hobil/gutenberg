...
class Model(object):
  ...
  def predict(self, X,y=None,statistics=False,labels=None):
  ...
  def predict_single(self,X, n=None, min_prob = None):
    """ works only for single prediction"""
    predictions = self.model.predict_proba(X).ravel()
    if set([True,False]) == set(self.model.classes_):
      # binary model
      idx_True = self.model.classes_[self.model.classes_]
      return predictions[idx_True]
    else:
      # multiclass model
      prediction = self.model.predict_proba(X).ravel()
      prediction2 = [(self.model.classes_[k],score) for k,score in enumerate(prediction) if score > min_prob]
      prediction2 = sorted(prediction2,key = lambda x: x[1], reverse = True)
      return prediction2[:n]
...
"""
DecisionTreeClassifier(BaseDecisionTree, ClassifierMixin):
RandomForestClassifier(ForestClassifier):
ForestClassifier(six.with_metaclass(ABCMeta, BaseForest,
ClassifierMixin)):
BaseForest(six.with_metaclass(ABCMeta, BaseEnsemble))
BaseEnsemble(BaseEstimator, MetaEstimatorMixin):
"""